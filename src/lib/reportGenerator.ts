...
import { STOP_WORDS } from "./analyzer.ts";
...
export function generateReportHtml(result: AnalysisResult, layout: "complete" | "executive" | "technical" = "complete"): string {
  ...
  if (layout === "executive") {
    tocHtml = `
      <div class="toc">
        ...
        <a href="#scores">1. Quality &amp; SEO Scores</a>
        <a href="#grades">2. Category Grade Card</a>
        <a href="#insights">3. Strengths &amp; Weaknesses</a>
        <a href="#action">4. Recommended Action Roadmap</a>
      </div>
    `;
  } else if (layout === "technical") {
    tocHtml = `
      <div class="toc">
        <h3>Technical Audit Contents</h3>
        <a href="#meta">1. Head Metadata &amp; Technical Details</a>
        <a href="#verification">2. Integration Verification Checklist</a>
        <a href="#meta-gen">3. AI Optimizations &amp; Schema Generator</a>
        <a href="#structure">4. Heading Outline Structure</a>
      </div>
    `;
  }

  const headerLayoutBadge = layout === "complete" ? "Complete Audit Report" : (layout === "executive" ? "Executive Summary" : "Developer Technical Audit");

  // Core Sections Builder based on selected Layout
  let reportBodyHtml = "";
  ...
  if (layout === "complete") {
    reportBodyHtml = `
      ...
      <div class="section" id="phrases">
        ...
        <div class="phrase-list">
          <div>
            <h3 style="margin-bottom:12px;font-size:15px;color:#475569;">Bigrams</h3>
          ...
          <div>
            <h3 style="margin-bottom:12px;font-size:15px;color:#475569;">Trigrams</h3>
      ...
      <div class="section" id="text">
        <h2><span class="num">11</span> Full Extracted Page Text</h2>
        ${paragraphHtml || '<p class="empty">No paragraph content was extractable from the page HTML.</p>'}
      </div>

      <div class="section" id="action">
        <h2><span class="num">12</span> Priority Action Plan</h2>
      ...
      <div class="section" id="verification">
        <h2><span class="num">13</span> Verification Checklist</h2>
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          ...
          <tbody>
            ${result.verificationChecks.map(c => `<tr>
              <td style="padding:8px;"><strong>${escapeHtml(c.label)}</strong></td>
              <td style="padding:8px;">${escapeHtml(c.expected)}</td>
              <td style="padding:8px;color:${c.passed ? "#059669" : "#dc2626"};">${escapeHtml(c.actual)}</td>
              <td style="padding:8px;text-align:center;">${c.passed ? "&#10004;" : "&#10008;"}</td>
            </tr>`).join("")}
          </tbody>
        </table>
      </div>
      ...
      <div class="section" id="insights">
        ...
        <h3 style="margin-bottom:10px;font-size:15px;color:#059669;">Strengths</h3>
        <ul class="insight-list">${insightList(strengths)}</ul>
        <h3 style="margin:20px 0 10px;font-size:15px;color:#dc2626;">Weaknesses</h3>
        <ul class="insight-list">${insightList(weaknesses)}</ul>
        <h3 style="margin:20px 0 10px;font-size:15px;color:#0ea5e9;">Recommendations</h3>
    ...
    reportBodyHtml = `
      <div class="section" id="scores">
        <h2><span class="num">1</span> Quality &amp; SEO Scores</h2>
        <div class="score-grid">
          <div class="score-card">
            <div class="val" style="color:${seoColor}">${result.seoScore}</div>
            <div class="label">SEO Score (0-100)</div>
      ...
      <div class="section" id="grades">
        <h2><span class="num">2</span> Category Grade Card</h2>
        <div style="text-align:center;margin-bottom:20px;">
          <span class="grade-box grade-${result.gradeCard.overallGrade.charAt(0)}">${result.gradeCard.overallGrade}</span>
          <p style="color:#64748b;font-size:13px;margin-top:8px;">Overall Quality Grade</p>
        </div>
        <div class="grade-grid">
          ${result.gradeCard.entries.map(e => {
            const pct = Math.round((e.score / e.maxScore) * 100);
      ...
      <div class="section" id="insights">
        <h2><span class="num">3</span> Strengths &amp; Weaknesses</h2>
        <h3 style="margin-bottom:10px;font-size:15px;color:#059669;">Primary Strengths</h3>
        <ul class="insight-list">${insightList(strengths.slice(0, 4))}</ul>
        <h3 style="margin:20px 0 10px;font-size:15px;color:#dc2626;">Primary Weaknesses</h3>
        <ul class="insight-list">${insightList(weaknesses.slice(0, 4))}</ul>
      </div>

      <div class="section" id="action">
        <h2><span class="num">4</span> Recommended Action Roadmap</h2>
        <p style="color:#64748b;font-size:13px;margin-bottom:16px;">Key actions to execute in order of impact:</p>
        ${result.actionPlan.slice(0, 5).map(item => `<div class="action-item">
          <div class="action-num">${item.step}</div>
          <div>
            <div style="display:flex;justify-content:space-between;align-items:start;gap:8px;">
              <strong style="font-size:14px;">${escapeHtml(item.title)}</strong>
            ...
            <p style="font-size:13px;color:#475569;margin-top:4px;">${escapeHtml(item.detail)}</p>
          </div>
        </div>`).join("")}
      </div>
    `;
    ...
    reportBodyHtml = `
      <div class="section" id="meta">
        <h2><span class="num">1</span> Head Metadata &amp; Technical Details</h2>
        <div class="meta-row"><span class="k">Title Tag (${parsed.title?.length ?? 0} chars)</span><span class="v">${parsed.title ? escapeHtml(parsed.title) : "Missing"}</span></div>
        <div class="meta-row"><span class="k">Meta Description (${parsed.metaDescription?.length ?? 0} chars)</span><span class="v">${parsed.metaDescription ? escapeHtml(parsed.metaDescription) : "Missing"}</span></div>
        <div class="meta-row"><span class="k">Canonical URL</span><span class="v">${parsed.canonical ? escapeHtml(parsed.canonical) : "Missing"}</span></div>
        <div class="meta-row"><span class="k">Viewport Configuration</span><span class="v">${parsed.viewport ? escapeHtml(parsed.viewport) : "Missing"}</span></div>
        <div class="meta-row"><span class="k">HTTP Response Code</span><span class="v">${parsed.statusCode}</span></div>
        <div class="meta-row"><span class="k">HTML Content Size</span><span class="v">${(parsed.rawHtmlLength / 1024).toFixed(1)} KB</span></div>
        <div class="meta-row"><span class="k">SSL Active</span><span class="v">${parsed.finalUrl.startsWith("https://") ? "Yes" : "No"}</span></div>
      </div>

      <div class="section" id="verification">
        <h2><span class="num">2</span> Integration Verification Checklist</h2>
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          <thead>
            <tr style="border-bottom:2px solid #e2e8f0;">
              <th style="text-align:left;padding:8px;">Check</th>
      ...
      <div class="section" id="meta-gen">
        <h2><span class="num">3</span> AI Optimizations &amp; Schema Generator</h2>
        <h3 style="font-size:14px;margin-bottom:8px;">Schema.org JSON-LD (Org &amp; Website)</h3>
        <pre class="code-block">${escapeHtml(result.aiFixes.schemaJsonLd)}</pre>
        <h3 style="font-size:14px;margin:20px 0 8px;">Meta Snippet Output</h3>
        <pre class="code-block">${escapeHtml(result.aiFixes.metaTagsCode)}</pre>
      </div>

      <div class="section" id="structure">
        <h2><span class="num">4</span> Heading Outline Structure</h2>
        ${headingHtml || '<p class="empty">No headings found on this page</p>'}
      </div>
    `;
  }
...
<title>${escapeHtml(headerLayoutBadge)} — ${escapeHtml(domain)} — ${formatDate()}</title>
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .grade-box.grade-A { background: #d1fae5; color: #059669; }
  .grade-box.grade-B { background: #cffafe; color: #0891b2; }
  .grade-box.grade-C { background: #fef3c7; color: #d97706; }
  .grade-box.grade-D { background: #fed7aa; color: #ea580c; }
  .grade-box.grade-F { background: #fee2e2; color: #dc2626; }
  ...
  .code-block { background: #1e293b; color: #6ee7b7; border-radius: 8px; padding: 12px; font-family: monospace; font-size: 11px; margin-top: 8px; overflow-x: auto; white-space: pre-wrap; word-break: break-all; }
  ...
  .action-impact { color: #10b981; font-weight: 600; font-size: 12px; }
  @media print {
...
<div class="container">
  <div class="header">
    <span class="badge live">${escapeHtml(headerLayoutBadge)}</span>
    ${parsed.isClientRendered ? '<span class="badge spa">SPA Client-rendered</span>' : ""}
    <h1>${escapeHtml(parsed.title || domain)}</h1>
    <div class="url">${escapeHtml(parsed.finalUrl)}</div>
    <div class="date">Generated on ${formatDate()} &middot; SentinelQA</div>
  </div>
  ...
  <div class="footer">
    Generated by SentinelQA Intelligence Platform &middot; ${escapeHtml(domain)}<br>
    This report contains layout metrics as captured by the crawlers.
  </div>
</div>
</body>
</html>`;
...
export function downloadReport(result: AnalysisResult, layout: "complete" | "executive" | "technical" = "complete"): void {
  const html = generateReportHtml(result, layout);
  ...
  const filename = `seo-report-${domain}-${layout}-${dateStr}.html`;
